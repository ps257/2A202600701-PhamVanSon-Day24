...
def pii_scan(text: str, analyzer=None, anonymizer=None) -> dict:
    ...
    # Only high-confidence results; exclude noisy types often false-positive in VN text
    EXCLUDED_TYPES = {"DATE_TIME", "NRP", "LOCATION", "ORGANIZATION"}
    SCORE_THRESHOLD = 0.9  # Keeps VN_CCCD/VN_PHONE (0.9) and EMAIL (1.0), filters PERSON (0.85)
    # VN_CCCD has two patterns: 12-digit (0.9) and 9-digit CMND (0.7) — allow lower threshold
    VN_CCCD_THRESHOLD = 0.65

    raw_results = analyzer.analyze(text=text, language=PRESIDIO_LANGUAGE)
    results = [
        r for r in raw_results
        if r.entity_type not in EXCLUDED_TYPES and (
            (r.entity_type == "VN_CCCD" and r.score >= VN_CCCD_THRESHOLD)
            or r.score >= SCORE_THRESHOLD
        )
    ]
...
def setup_nemo_rails():
    ...
    config_path = os.path.join(GUARDRAILS_CONFIG_DIR, "config.yml")
    with open(config_path, "r", encoding="utf-8") as f:
        cfg_data = yaml.safe_load(f)

    # Automatically set the model based on environment variable
    key = os.getenv("OPENAI_API_KEY", "")
    if key.startswith("sk-or-"):
        target_model = os.getenv("MODEL_API_KEY", "openrouter/free")
    else:
        target_model = "gpt-4o-mini"

    current_model = cfg_data["models"][0]["model"]
    if current_model != target_model:
        cfg_data["models"][0]["model"] = target_model
...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # NeMo may return a dict {"role": ..., "content": ...} or a plain string
    response = raw["content"] if isinstance(raw, dict) else raw
    refuse_keywords = ["xin lỗi", "không thể", "không được phép", "i cannot", "i'm sorry", "truy cập bị chặn"]
    blocked = any(kw in response.lower() for kw in refuse_keywords)

    # Fallback: keyword/regex detection when NeMo LLM call is unavailable (empty response)
    # This implements "defense-in-depth" — catches obvious threats without LLM
    if not blocked:
        import re
        text_lower = text.lower()
        JAILBREAK_PATTERNS = [
            r"ignore\s+(previous|all|your)\s+(instructions?|rules?|guidelines?|system)",
            r"\bdan\b",                      # DAN jailbreak
            r"pretend\s+you\s+are",
            r"you\s+are\s+now\s+an?\s+(unrestricted|unlimited)",
            r"system\s+override",
            r"admin\s+command",
            r"forget\s+all\s+(hr|policy|rules?|instructions?)",
            r"bỏ\s+qua\s+tất\s+cả\s+hướng\s+dẫn",
            r"đóng\s+vai",                   # role-play jailbreak (VN)
            r"không\s+có\s+giới\s+hạn",     # "no restrictions" in VN
            r"tiết\s+lộ\s+(mật\s+khẩu|lương|thông\s+tin\s+nhân\s+viên)",
            r"(cccd|cmnd|số\s+điện\s+thoại|sdt)\s+(của|và)\s+(nhân\s+viên|người)",  # requesting others' PII
            r"cho\s+tôi\s+biết.*(cccd|cmnd|sdt|điện\s+thoại).*(nhân\s+viên)",  # ask for employee PII
            r"(tôi\s+là|tôi\s+là)\s+(ceo|giám\s+đốc|sếp).*(ra\s+lệnh|yêu\s+cầu)",  # authority impersonation
            r"tiết\s+lộ\s+lương",            # reveal salary
        ]
        OFF_TOPIC_PATTERNS = [
            r"bài\s+thơ",                    # poem
            r"nấu\s+phở|công\s+thức\s+(nấu|làm)",
            r"bitcoin|ethereum|crypto",
            r"phương\s+trình\s+vi\s+phân",   # differential equation
            r"phim\s+(marvel|hay\s+nhất)",
        ]
        PROMPT_INJECTION_PATTERNS = [
            r"ignore previous instructions",
            r"<!--\s*ignore",
            r"\[admin\s+(command|order)",
            r"print\s+all\s+(confidential|training|employee)",
            r"dump\s+all",
            r"system\s+instructions\s+and\s+context",
            r"in\s+ra\s+toàn\s+bộ\s+system",   # VN: "print all system instructions"
            r"toàn\s+bộ\s+system\s+instructions",
        ]
        all_patterns = JAILBREAK_PATTERNS + OFF_TOPIC_PATTERNS + PROMPT_INJECTION_PATTERNS
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    # NeMo may return a dict {"role": ..., "content": ...} or a plain string
    response = raw["content"] if isinstance(raw, dict) else raw
    refuse_keywords = ["xin lỗi", "không thể cung cấp", "i cannot", "tôi không thể", "chặn"]
...
def measure_p95_latency(test_inputs: list[str], n_runs: int = 20,
    ...
    def percentiles(times):
        ...
        return {
            ...
            "p95": round(s[int(n * 0.95)] if int(n * 0.95) < n else s[-1], 2),
...
if __name__ == "__main__":
    ...
    report_data = {
        "adversarial_pass_rate": round(pass_rate, 3),
        "p95_latency_ms": latency['total_ms']['p95'],
