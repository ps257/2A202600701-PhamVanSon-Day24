...
from config import TEST_SET_PATH, get_llm_client
...
def evaluate_ragas(questions: list[str], answers: list[str],
    ...
    try:
        ...
        # Set OpenRouter key nếu không có OpenAI key
        client, model = get_llm_client()
        if client and not os.getenv("OPENAI_API_KEY"):
            from config import OPEN_ROUTER_API_KEY, OPENROUTER_BASE_URL
        ...
        return {"faithfulness": float(agg["faithfulness"] or 0.0), 
                "answer_relevancy": float(agg["answer_relevancy"] or 0.0),
                "context_precision": float(agg["context_precision"] or 0.0), 
                "context_recall": float(agg["context_recall"] or 0.0), 
...
def failure_analysis(eval_results: list[EvalResult], bottom_n: int = 10) -> list[dict]:
    ...
    for r in eval_results:
        ...
        scored_results.append((avg_score, worst_metric, r))
        
    scored_results.sort(key=lambda x: x[0])
    bottom_n_results = scored_results[:bottom_n]
    
    failures = []
    for avg, worst_metric, r in bottom_n_results:
