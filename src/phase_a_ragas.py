...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    dist_failures = {"factual": 0, "multi_hop": 0, "adversarial": 0}
    for metric in matrix:
        for dist in dist_failures:
            dist_failures[dist] += matrix[metric][dist]
    dominant_dist = max(dist_failures, key=dist_failures.get)

    # Find dominant failure metric
    metric_failures = {metric: sum(matrix[metric].values()) for metric in matrix}
    dominant_metric = max(metric_failures, key=metric_failures.get)
