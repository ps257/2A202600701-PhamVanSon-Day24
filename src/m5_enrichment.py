...
def contextual_prepend(text: str, document_title: str = "") -> str:
    ...
    if OPENAI_API_KEY:
        try:
            ...
            print(f"  OpenAI contextual failed: {e}")
    # Simple fallback
    ...
    if OPENAI_API_KEY:
        try:
            ...
            resp = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": '{"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}'},
                    {"role": "user", "content": text},
                ],
                max_tokens=150,
            )
...
def enrich_chunks(
    ...
    # Bypass enrichment to save API credits and avoid rate limits on free models
    return []
